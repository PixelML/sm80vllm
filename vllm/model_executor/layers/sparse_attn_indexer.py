...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        if current_platform.is_xpu():
            ...
            # SM80/SM121 Triton fallback (DeepGEMM unavailable). Downstream
            # topk reads only up to `seq_lens`, so size the buffer to the
            # active batch max rather than the configured model max.
            assert padded_q_scale is None, (
                "FP4 indexer cache is not supported on the Triton fallback"
            )
